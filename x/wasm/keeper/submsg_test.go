...
func TestDispatchSubMsgErrorHandling(t *testing.T) {
	...
	ctx, keepers := CreateTestInput(t, false, ReflectFeatures)
	ctx = ctx.WithGasMeter(stypes.NewInfiniteGasMeter())
	ctx = ctx.WithBlockGasMeter(stypes.NewInfiniteGasMeter())
	...
	cases := map[string]struct {
		...
		"send tokens": {
			submsgID:         5,
			msg:              validBankSend,
			resultAssertions: []assertion{assertReturnedEvents(3), assertGasUsed(112000, 130000)},
		},
		"not enough tokens": {
			submsgID:    6,
			msg:         invalidBankSend,
		...
		"not enough tokens with limit": {
			...
			gasLimit:    &subGasLimit,
			// uses same gas as call without limit (note we do not charge the 40k on reply)
			resultAssertions: []assertion{assertGasUsed(75000, 79000), assertErrorString("codespace: sdk, code: 5")},
		},
	...
	for name, tc := range cases {
		t.Run(name, func(t *testing.T) {
			...
			execCtx := ctx.WithGasMeter(stypes.NewGasMeter(ctxGasLimit))
			defer func() {
				if tc.isOutOfGasPanic {
					r := recover()
					require.NotNil(t, r, "expected panic")
					if _, ok := r.(stypes.ErrorOutOfGas); !ok {
